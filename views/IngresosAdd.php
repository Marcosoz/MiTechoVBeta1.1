...
<span id="el_ingresos_cooperativa_id">
    <select
        ...
        data-select2-id="fingresosadd_x_cooperativa_id"
        <?php } ?>
        data-table="ingresos"
...
<?php if (!$Page->cooperativa_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fingresosadd", function() {
    var options = { name: "x_cooperativa_id", selectId: "fingresosadd_x_cooperativa_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fingresosadd.lists.cooperativa_id?.lookupOptions.length) {
        options.data = { id: "x_cooperativa_id", form: "fingresosadd" };
    } else {
        options.ajax = { id: "x_cooperativa_id", form: "fingresosadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
