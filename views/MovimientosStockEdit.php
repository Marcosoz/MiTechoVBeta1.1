...
<span id="el_movimientos_stock_cooperativa_id">
    <select
        ...
        data-select2-id="fmovimientos_stockedit_x_cooperativa_id"
        <?php } ?>
...
<?php if (!$Page->cooperativa_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fmovimientos_stockedit", function() {
    var options = { name: "x_cooperativa_id", selectId: "fmovimientos_stockedit_x_cooperativa_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmovimientos_stockedit.lists.cooperativa_id?.lookupOptions.length) {
        options.data = { id: "x_cooperativa_id", form: "fmovimientos_stockedit" };
    } else {
        options.ajax = { id: "x_cooperativa_id", form: "fmovimientos_stockedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
