...
<input type="hidden" data-table="stock" data-field="x_cooperativa_id" data-hidden="1" name="x_cooperativa_id" id="x_cooperativa_id" value="<?= HtmlEncode($Page->cooperativa_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_stock_cooperativa_id">
    <select
        ...
        data-select2-id="fstockedit_x_cooperativa_id"
        <?php } ?>
        data-table="stock"
...
<?php if (!$Page->cooperativa_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fstockedit", function() {
    var options = { name: "x_cooperativa_id", selectId: "fstockedit_x_cooperativa_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fstockedit.lists.cooperativa_id?.lookupOptions.length) {
        options.data = { id: "x_cooperativa_id", form: "fstockedit" };
    } else {
        options.ajax = { id: "x_cooperativa_id", form: "fstockedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.stock.fields.cooperativa_id.selectOptions);
