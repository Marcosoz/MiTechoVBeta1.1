...
<span id="el_horas_trabajadas_cooperativa_id">
    <select
        ...
        data-select2-id="fhoras_trabajadasedit_x_cooperativa_id"
        <?php } ?>
...
<?php if (!$Page->cooperativa_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fhoras_trabajadasedit", function() {
    var options = { name: "x_cooperativa_id", selectId: "fhoras_trabajadasedit_x_cooperativa_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fhoras_trabajadasedit.lists.cooperativa_id?.lookupOptions.length) {
        options.data = { id: "x_cooperativa_id", form: "fhoras_trabajadasedit" };
    } else {
        options.ajax = { id: "x_cooperativa_id", form: "fhoras_trabajadasedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
