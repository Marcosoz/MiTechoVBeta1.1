...
<span id="el_compras_cooperativa_id">
    <select
        ...
        data-select2-id="fcomprasadd_x_cooperativa_id"
        <?php } ?>
        data-table="compras"
...
<?php if (!$Page->cooperativa_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fcomprasadd", function() {
    var options = { name: "x_cooperativa_id", selectId: "fcomprasadd_x_cooperativa_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcomprasadd.lists.cooperativa_id?.lookupOptions.length) {
        options.data = { id: "x_cooperativa_id", form: "fcomprasadd" };
    } else {
        options.ajax = { id: "x_cooperativa_id", form: "fcomprasadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
