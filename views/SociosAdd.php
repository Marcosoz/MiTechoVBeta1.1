...
<span id="el_socios_cupo">
    <select
        ...
        data-select2-id="fsociosadd_x_cupo"
        <?php } ?>
        data-table="socios"
        data-field="x_cupo"
...
<script<?= Nonce() ?>>
loadjs.ready("fsociosadd", function() {
    var options = { name: "x_cupo", selectId: "fsociosadd_x_cupo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsociosadd.lists.cupo?.lookupOptions.length) {
        options.data = { id: "x_cupo", form: "fsociosadd" };
    } else {
        options.ajax = { id: "x_cupo", form: "fsociosadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
