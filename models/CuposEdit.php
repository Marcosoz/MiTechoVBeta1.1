...
class CuposEdit extends Cupos
{
    ...
    // Page object name
    public string $PageObjName = "CuposEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "CuposEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("CuposList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "CuposList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "CuposList") {
                            ...
                            $returnUrl = "CuposList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues(): void
    {
        ...
        if (!$this->cupo->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->cupo->setFormValue($val, true, $validate);
            }
        }
    }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->cupo->EditValue = $this->cupo->CurrentValue;
            $this->cupo->PlaceHolder = RemoveHtml($this->cupo->caption());
            if (strval($this->cupo->EditValue) != "" && is_numeric($this->cupo->EditValue)) {
                $this->cupo->EditValue = FormatNumber($this->cupo->EditValue, null);
            }
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if (!CheckInteger($this->cupo->FormValue)) {
                $this->cupo->addErrorMessage($this->cupo->getErrorMessage(false));
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // cupo
        $this->cupo->setDbValueDef($newRow, $this->cupo->CurrentValue, $this->cupo->ReadOnly);
        return $newRow;
    }
    ...
    {
        if (isset($row['cupo'])) { // cupo
            $this->cupo->CurrentValue = $row['cupo'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("CuposList"), "", $this->TableVar, true);
        $pageId = "edit";
